...
// Inicialização do Swiper para a apresentação de fotos
const photoSlider = new Swiper('.mil-photo-slider', {
    slidesPerView: 3, // Quantidade de slides visíveis
    spaceBetween: 30, // Espaçamento entre os slides
    loop: true, // Loop infinito da galeria
    autoplay: { // Autoplay para que as fotos passem automaticamente
        delay: 5000, // Tempo de atraso entre os slides (5 segundos)
        disableOnInteraction: false, // Continua o autoplay mesmo após interação do usuário
    },
    speed: 800, // Velocidade da transição
    navigation: {
        nextEl: '.mil-photo-next',
        prevEl: '.mil-photo-prev',
    },
    breakpoints: {
    ...
    ***************************/
    ...
    $container.after('<div class="mil-load-more mil-up"><button id="load-more">VEJA MAIS</button></div>');
...
// Coleta todas as imagens ao carregar a página
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.galeria .foto img').forEach(img => {
    imagens.push(img.getAttribute('data-full'));
  });
});
